...
class Jabber
{
	...
	function Connect()
	{
		...
		if ($this->CONNECTOR->OpenSocket($this->server, $this->port))
		{
			...
			$stream_txt = "<stream:stream to='{$this->server}' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'";
			if($this->show_version) $stream_txt .= " version='{$this->version}'";
			$stream_txt .= ">\n";
			$this->SendPacket($stream_txt);
			sleep(2);
